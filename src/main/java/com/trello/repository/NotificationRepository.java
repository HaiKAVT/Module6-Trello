...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    Iterable<NotificationRepository> findByUserId(Long userId);

    @Modifying(clearAutomatically = true )
    @Query(value = "update notification join notification_receiver nr on notification.id = nr.notification_id set notification.status = true " +
            "where nr.receiver_id = ?1",nativeQuery = true)
